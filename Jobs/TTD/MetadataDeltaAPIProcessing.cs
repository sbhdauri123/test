...
{
    [Export("TTD-MetadataDeltaAPIDataLoad", typeof(IDragoJob))]
    public class MetadataDeltaAPIProcessing : Framework.BaseFrameworkJob, IDragoJob
    {
        ...
        private void ProcessSqlEtlAndMarkQueueComplete(IFileItem queueItem, Uri path)
        {
            ...
            LogMessage(LogLevel.Info, $"Start loading TTD Metadata into SQL Dimension DB file->path:{path};guid:{queueItem.FileGUID};");

            string jobGUID = this.JED.JobGUID.ToString();
            var etl = new Greenhouse.DAL.ETLProvider();
            etl.SetJobLogGUID(jobGUID);

            etl.LoadTTDDeltaAPIData(queueItem, path, base.SourceId, base.IntegrationId, CurrentIntegration.CountryID);

            LogMessage(LogLevel.Info, "End loading TTD Metadata into SQL Dimension DB");
        ...
        {
            LogMessage(LogLevel.Info, $"Start loading TTD Metadata file into Databricks->path:{path};guid:{queueItem.FileGUID};");
            ...
            LogMessage(LogLevel.Info, "End loading TTD Metadata into Databricks");
        }
        ...
        private Uri GetStageFilePath()
        {
            ...
            string[] paths = [_queueItem.EntityID.ToLower(), GetDatedPartition(_queueItem.FileDate), GetHourPartition((int)_queueItem.FileDateHour)];
        ...
        {
            _exceptionCount++;
            LogMessage(LogLevel.Error, $"ERROR->FileGUID: {_queueItem.FileGUID} Something went wrong with the Databricks workflow for TTD Metadata Job ID: {_databricksEtlJob.DatabricksJobID} - Queue ID: {queueID}");
            MarkQueueError();
        }
